...
import type { ExerciseWithSettings } from "@/core/types/workout";
  ...
  if (ownedIds.length > 0) {
    const otherSets = await prisma.exerciseSet.findMany({
      where: {
        ...
        deletedAt: null,
        session: { userId: { not: userId }, deletedAt: null },
      },
...
export async function getHiddenExercisesForUser(userId: string) {
  const settings = await prisma.userExerciseSetting.findMany({
    ...
    include: {
      exercise: { select: { id: true, name: true, muscleGroup: true, createdByUserId: true } },
    },
  });
...
export async function getCommunityExercisesForUser(userId: string) {
  ...
  const alreadyLinkedIds = new Set([...userSettings.map((s) => s.exerciseId)]);
...
export async function deleteExerciseData(userId: string, exerciseId: string) {
  ...
  // Soft-delete all ExerciseSets belonging to this user for this exercise
  await prisma.exerciseSet.updateMany({
    where: { exerciseId, deletedAt: null, session: { userId } },
    data: { deletedAt: new Date() },
  });

  // Always mark hidden — data is preserved via soft-delete for recovery
