...
import type {
  ...
  SeriesRuleType as SeriesRuleTypeEnum,
...
export async function getTrackedGroupsByDate(
  ...
  const sessions = await prisma.workoutSession.findMany({
    ...
    select: {
      date: true,
      sets: { where: { deletedAt: null }, select: { exercise: { select: { muscleGroup: true } } } },
    },
  });
...
export async function createOneOffBlock(userId: string, date: string, blockType: BlockTypeEnum) {
...
export async function createAutoPromotedBlock(
  ...
  const existing = await prisma.plannedWorkout.findFirst({
    where: { userId, deletedAt: null, date: new Date(date + "T12:00:00"), blockType },
  });
...
export async function deleteAutoPromotedBlock(
  ...
  await prisma.plannedWorkout.updateMany({
    where: { userId, deletedAt: null, date: new Date(date + "T12:00:00"), blockType, isAutoPromoted: true },
...
export async function deleteSeries(userId: string, seriesId: string, fromDate?: string) {
  const where = fromDate
    ? { seriesId, userId, deletedAt: null as null, date: { gte: new Date(fromDate) } }
    : { seriesId, userId, deletedAt: null as null };

  await prisma.plannedWorkout.updateMany({ where, data: { deletedAt: new Date() } });

  // If deleting all, also soft-delete the series record
  if (!fromDate) {
    await prisma.plannedWorkoutSeries.updateMany({
      where: { id: seriesId, userId, deletedAt: null },
      data: { deletedAt: new Date() },
    });
  }
}
...
export async function updateBlock(userId: string, blockId: string, blockType: BlockTypeEnum) {
...
export async function updateSeries(userId: string, seriesId: string, config: SeriesConfig) {
  const today = new Date();
  today.setHours(0, 0, 0, 0);

  // Soft-delete future instances
  await prisma.plannedWorkout.updateMany({
    where: { seriesId, userId, deletedAt: null, date: { gte: today } },
    data: { deletedAt: new Date() },
  });
  ...
  if (pastDates.length > 0) {
    const existing = await prisma.plannedWorkout.findMany({
      where: { seriesId, userId, deletedAt: null, date: { lt: today } },
      select: { date: true },
    });
...
async function incrementSorryToken(userId: string, month: string): Promise<void> {
  const user = await prisma.user.findUnique({
    where: { id: userId }, select: { sorryTokenMax: true },
  });
  const max = user?.sorryTokenMax ?? 3;

  const existing = await prisma.userSorryToken.findUnique({
    where: { userId_month: { userId, month } },
  });

  if (!existing) {
    await prisma.userSorryToken.create({ data: { userId, month, usedCount: 1 } });
    return;
  }

  if (existing.usedCount >= max) {
    throw new Error("No SORRY tokens remaining this month.");
  }

  // Atomic conditional update — only increments if still under limit
  const updated = await prisma.userSorryToken.updateMany({
    where: { userId, month, usedCount: { lt: max } },
...
export async function getSorryTokensThisMonth(
  userId: string,
): Promise<{ usedCount: number; max: number; month: string }> {
  const d = new Date();
  const month = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
  const [token, user] = await Promise.all([
    prisma.userSorryToken.findUnique({ where: { userId_month: { userId, month } } }),
    prisma.user.findUnique({ where: { id: userId }, select: { sorryTokenMax: true } }),
  ]);
  return { usedCount: token?.usedCount ?? 0, max: user?.sorryTokenMax ?? 3, month };
}
...
export async function getStreakData(userId: string): Promise<StreakData> {
  ...
  const [sorryToken, userPrefs, allTimeSessionDates, allTimePlannedRaw] = await Promise.all([
    ...
    prisma.user.findUnique({
      where: { id: userId },
      select: { sorryTokenMax: true, sorryTokenMaxEditedMonth: true },
    }),
    prisma.workoutSession.findMany({
      where: { userId, deletedAt: null, sets: { some: { deletedAt: null } } },
      select: { date: true },
    }),
    // Include today — planned today counts as "planned so far"
    prisma.plannedWorkout.findMany({
      where: { userId, deletedAt: null, date: { lte: new Date(todayISO + "T23:59:59") } },
      select: { date: true, sorryExcused: true },
    }),
  ]);

  // All-time workout stats — all counts use distinct dates for apples-to-apples comparison
  const allTimeWorkedSet = new Set(
    allTimeSessionDates.map((s) => {
      const sd = s.date;
  ...
  const uniquePlannedDatesDesc = [...new Set(allPlannedYearRaw.map((pw) => dbDateToISO(pw.date)))]
    .sort()
    .reverse();
  ...
  const allSeries = await prisma.plannedWorkoutSeries.findMany({
    where: {
      userId,
      deletedAt: null,
      workouts: { some: { deletedAt: null, date: { gte: sixtyDaysAgo } } },
    },
  ...
  if (allSeries.length === 0) {
    return {
      streaks: [],
      sorryUsed,
      sorryRemaining: Math.max(0, sorryMax - sorryUsed),
      sorryMax,
      canEditSorryMax,
      month,
      generalStreak,
  ...
  for (const series of allSeries) {
    const cutoffDate =
      series.streakResetDate && series.streakResetDate > series.startDate
...
/** Soft-delete a series block and reset the series streak */
...
export async function setSorryTokenMax(
  userId: string,
  newMax: number,
