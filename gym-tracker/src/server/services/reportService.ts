...
import { getRangeStart } from "@/core/utils/date";
import { isAssistedExercise } from "@/core/domain/workoutDiff";
import type { TimeRange, MetricPoint, ExerciseProgressPoint, PRRecord } from "@/core/types/metrics";
  ...
  for (const session of sessions) {
    ...
    for (const set of Object.values(latestBySetNumber)) {
      ...
      if (weight !== null) {
        if (isAssisted) {
          byDay[day].maxWeight =
            byDay[day].maxWeight === null ? weight : Math.min(byDay[day].maxWeight, weight);
        } else {
...
export async function getPersonalRecords(userId: string): Promise<PRRecord[]> {
  // Single query: fetch all non-cardio sets with exercise info
  const allSets = await prisma.exerciseSet.findMany({
    where: {
      deletedAt: null,
      session: { userId, deletedAt: null },
      exercise: { muscleGroup: { not: MuscleGroup.CARDIO } },
    },
    include: {
  ...
  // Group by exercise, find best set per exercise in JS
  const byExercise = new Map<
    string,
    { exercise: (typeof allSets)[0]["exercise"]; sets: (typeof allSets) }
  >();
  for (const set of allSets) {
    ...
    if (existing) {
      ...
      byExercise.set(set.exerciseId, { exercise: set.exercise, sets: [set] });
    }
  }
  ...
  for (const [, { exercise, sets }] of byExercise) {
    ...
    // Sort to find best set (same logic as before, but in JS)
    sets.sort((a, b) => {
      if (exercise.isBodyweight) {
        return b.reps - a.reps || a.recordedAt.getTime() - b.recordedAt.getTime();
      }
      if (assisted) {
        const aKg = a.weightKg !== null ? Number(a.weightKg) : Infinity;
        const bKg = b.weightKg !== null ? Number(b.weightKg) : Infinity;
        return aKg - bKg || b.reps - a.reps || a.recordedAt.getTime() - b.recordedAt.getTime();
      }
      const aKg = a.weightKg !== null ? Number(a.weightKg) : -Infinity;
      const bKg = b.weightKg !== null ? Number(b.weightKg) : -Infinity;
      return bKg - aKg || b.reps - a.reps || a.recordedAt.getTime() - b.recordedAt.getTime();
    });
...
export async function getExerciseStatCards(userId: string): Promise<ExerciseStatCard[]> {
  const sessions = await prisma.workoutSession.findMany({
    ...
    select: {
      date: true,
      sets: {
        where: { deletedAt: null, exercise: { muscleGroup: { not: MuscleGroup.CARDIO } } },
        select: {
          setNumber: true,
          reps: true,
          weightKg: true,
