...
import { refreshAccessToken, fetchMeasures } from "@/server/withings";
import { encryptIfKeySet, decryptOrPassthrough } from "@/server/encryption";
...
export async function syncWithingsIfNeeded(userId: string): Promise<void> {
  ...
  let accessToken = decryptOrPassthrough(connection.accessToken);
  let refreshToken = decryptOrPassthrough(connection.refreshToken);
  ...
  if (expiresAt.getTime() - Date.now() < TOKEN_REFRESH_THRESHOLD_MS) {
    try {
      ...
      await prisma.withingsConnection.update({
        where: { userId },
        data: {
          accessToken: encryptIfKeySet(refreshed.accessToken),
          refreshToken: encryptIfKeySet(refreshed.refreshToken),
  ...
  const existingGrpIds = await prisma.bodyMetricEntry
    .findMany({
      where: { userId, deletedAt: null, withingsMeasureGrpId: { not: null } },
...
export async function storeWithingsConnection(
  ...
  const encAccess = encryptIfKeySet(tokens.accessToken);
  const encRefresh = encryptIfKeySet(tokens.refreshToken);
