...
import { getPersonalRecords } from "@/server/services/reportService";
import { getLatestBodyMetric } from "@/server/services/metricsService";
import { getCumulativeVolume, computeBadges } from "@/server/services/progressService";
import type {
  FriendSummary,
  FriendProfileData,
  WorkoutFeedEntry,
...
export async function resolveFriendVisibility(
  ...
  const [friendSettings, friendOverride] = await Promise.all([
    ...
    prisma.friendPrivacyOverride.findUnique({
      where: { userId_friendId: { userId: friendId, friendId: viewerId } },
    }),
  ]);
...
export async function sendFriendRequest(
  ...
  if (existing) {
    ...
    // DECLINED — allow re-sending by soft-deleting and recreating
...
export async function removeFriend(userId: string, friendId: string) {
  ...
  // Soft-delete privacy overrides for this pair
...
export async function getPendingReceived(userId: string) {
  return prisma.friendship.findMany({
    where: { receiverId: userId, deletedAt: null, status: "PENDING" },
...
export async function getPendingSent(userId: string) {
  return prisma.friendship.findMany({
    where: { senderId: userId, deletedAt: null, status: "PENDING" },
...
export async function getFriendsWithStats(
  ...
  const profiles = await Promise.all(friends.map((f) => getFriendProfileData(userId, f.userId)));
  return profiles
...
export async function getFriendProfileData(
  ...
  const [
    ...
    prisma.workoutSession.count({ where: { userId: friendId, deletedAt: null } }),
...
export async function toggleFistBump(
  ...
  // IDOR guard: verify the session owner is an accepted friend (or the user themselves)
  ...
  if (session.userId !== userId) {
    ...
    if (!friendship) throw new Error("Not authorized to fist bump this session.");
  }
...
export async function getSocialBadgeCounts(
  ...
  const [requests, friendships, fistBumps] = await Promise.all([
    prisma.friendship.count({
      where: { receiverId: userId, deletedAt: null, status: "PENDING", createdAt: { gt: lastSeenRequests } },
    }),
...
export async function markSocialSeen(userId: string, section: "requests" | "feed"): Promise<void> {
  const now = new Date();
...
export async function getFriendsFeed(viewerId: string, days = 14): Promise<WorkoutFeedEntry[]> {
  ...
  for (const pw of planned) {
    const dateStr =
      pw.date instanceof Date ? pw.date.toISOString().split("T")[0] : String(pw.date).split("T")[0];
