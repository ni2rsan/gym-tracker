...
export {
  ...
  STAGE_THRESHOLDS,
  getGardenState,
} from "@/core/domain/gardenUtils";
export type { TreeState } from "@/core/domain/gardenUtils";
...
async function computeStardustFromHistory(userId: string): Promise<number> {
  ...
  const isBodyweightById = new Map(exercises.map((e) => [e.id, e.muscleGroup === "BODYWEIGHT"]));
  ...
  const prevSetsMap: Record<
    string,
    { setNumber: number; reps: number; weightKg: number | null }[]
  > = {};
  let total = 0;
  ...
  for (const session of sessions) {
    ...
    const byExercise: Record<
      string,
      Map<number, { reps: number; weightKg: number | null; recordedAt: Date }>
    > = {};
