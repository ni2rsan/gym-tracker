...
import { BADGE_CATALOG } from "@/core/constants/badges";
...
export async function getWeeklyVolumeComparison(userId: string): Promise<WeeklyVolumeComparison> {
  ...
  const sessions = await prisma.workoutSession.findMany({
    where: { userId, deletedAt: null, date: { gte: lastMon, lt: nextMon } },
    include: {
      sets: {
...
export async function getBiggestJump(userId: string, days = 30): Promise<BiggestJumpResult | null> {
  ...
  const sessions = await prisma.workoutSession.findMany({
    where: { userId, deletedAt: null, date: { gte: previousStart } },
    include: {
      sets: {
        where: { deletedAt: null },
        include: {
          exercise: { select: { id: true, name: true, muscleGroup: true, isBodyweight: true } },
        },
      },
    },
  });
...
export async function getCumulativeVolume(userId: string): Promise<number> {
  // Use DISTINCT ON to deduplicate append-only ExerciseSet rows:
  // only the latest recordedAt per (sessionId, exerciseId, setNumber) counts.
  const result = await prisma.$queryRaw<[{ total: bigint | null }]>`
    WITH latest AS (
      SELECT DISTINCT ON (es."sessionId", es."exerciseId", es."setNumber")
        es."weightKg", es."reps"
      FROM "ExerciseSet" es
      ...
      WHERE ws."userId" = ${userId}
        ...
        AND es."weightKg" > 0
      ORDER BY es."sessionId", es."exerciseId", es."setNumber", es."recordedAt" DESC
    )
    SELECT SUM("weightKg" * "reps") as total FROM latest
  `;
...
export async function getTotalWorkoutCount(userId: string): Promise<number> {
  return prisma.workoutSession.count({ where: { userId, deletedAt: null } });
}

// ─── Recent PRs ───────────────────────────────────────────────────────────────

export function getRecentPRs(prs: PRRecord[], limit = 3): PRRecord[] {
  return [...prs].sort((a, b) => b.achievedOn.localeCompare(a.achievedOn)).slice(0, limit);
}
...
export function computeBadges(inputs: BadgeInputs): UserBadge[] {
  ...
  const maxWeight = Math.max(
    0,
    ...prs.filter((p) => p.maxWeightKg != null && !p.isAssisted).map((p) => Number(p.maxWeightKg)),
  );
  ...
  const checks: Record<string, { unlocked: boolean; progress?: number; progressLabel?: string }> = {
    ...
    streak_10: {
      ...
      progressLabel: `${Math.min(bestStreak, 10)} / 10`,
    },
    streak_30: {
      unlocked: bestStreak >= 30,
      progress: Math.min(1, bestStreak / 30),
      progressLabel: `${Math.min(bestStreak, 30)} / 30`,
    },
    streak_50: {
      unlocked: bestStreak >= 50,
      progress: Math.min(1, bestStreak / 50),
      progressLabel: `${Math.min(bestStreak, 50)} / 50`,
    },
    streak_75: {
      unlocked: bestStreak >= 75,
      progress: Math.min(1, bestStreak / 75),
      progressLabel: `${Math.min(bestStreak, 75)} / 75`,
    },
    streak_100: {
      unlocked: bestStreak >= 100,
      progress: Math.min(1, bestStreak / 100),
      progressLabel: `${Math.min(bestStreak, 100)} / 100`,
    },

    // Consistency
    first_steps: {
      unlocked: thisWeekCount >= 3,
      progress: Math.min(1, thisWeekCount / 3),
      progressLabel: `${Math.min(thisWeekCount, 3)} / 3`,
    },
    week_warrior: { unlocked: isWeekWarrior },
    month_machine: {
      unlocked: consistency30 >= 90,
      progress: Math.min(1, consistency30 / 90),
      progressLabel: `${Math.round(consistency30)}%`,
    },

    // Strength
    first_20kg: {
      unlocked: maxWeight >= 20,
      progress: Math.min(1, maxWeight / 20),
      progressLabel: `${Math.round(maxWeight)} / 20 kg`,
    },
    half_zentner: {
      unlocked: maxWeight >= 50,
      progress: Math.min(1, maxWeight / 50),
      progressLabel: `${Math.round(maxWeight)} / 50 kg`,
    },
    zentner: {
      unlocked: maxWeight >= 100,
      progress: Math.min(1, maxWeight / 100),
      progressLabel: `${Math.round(maxWeight)} / 100 kg`,
    },

    // Volume
    vol_1t: {
      unlocked: cumulativeVolume >= 1000,
      progress: Math.min(1, cumulativeVolume / 1000),
      progressLabel: `${Math.round(cumulativeVolume)} / 1.000 kg`,
    },
    vol_10t: {
      unlocked: cumulativeVolume >= 10000,
      progress: Math.min(1, cumulativeVolume / 10000),
      progressLabel: `${Math.round(cumulativeVolume / 1000)}t / 10t`,
    },
    vol_100t: {
      unlocked: cumulativeVolume >= 100000,
      progress: Math.min(1, cumulativeVolume / 100000),
      progressLabel: `${Math.round(cumulativeVolume / 1000)}t / 100t`,
    },

    // Special
    club_100: {
      unlocked: totalWorkouts >= 100,
      progress: Math.min(1, totalWorkouts / 100),
      progressLabel: `${Math.min(totalWorkouts, 100)} / 100`,
    },
...
export async function hasAllrounderThisWeek(userId: string): Promise<boolean> {
  ...
  const muscleGroups = await prisma.exerciseSet.findMany({
    where: {
      deletedAt: null,
      session: { userId, deletedAt: null, date: { gte: thisMon, lt: nextMon } },
    },
  ...
  return (
    groups.has(MuscleGroup.UPPER_BODY) &&
    groups.has(MuscleGroup.LOWER_BODY) &&
    groups.has(MuscleGroup.BODYWEIGHT)
  );
}
