import "server-only";
// APPEND-ONLY: No update or hard-delete on BodyMetricEntry rows. Soft-delete via deletedAt.
...
export async function getBodyMetrics(userId: string, range: TimeRange | "all" = "all") {
  const where =
    range === "all"
      ? { userId, deletedAt: null as null }
      : { userId, deletedAt: null as null, recordedAt: { gte: getRangeStart(range) } };
...
export async function getLatestBodyMetric(userId: string) {
  const [latestWeight, latestBodyFat, latestFatMass, latestMuscleMass] = await Promise.all([
    ...
    prisma.bodyMetricEntry.findFirst({
      where: { userId, deletedAt: null, bodyFatPct: { not: null } },
      orderBy: { recordedAt: "desc" },
      select: { bodyFatPct: true, recordedAt: true, source: true },
    }),
    prisma.bodyMetricEntry.findFirst({
      where: { userId, deletedAt: null, fatMassKg: { not: null } },
      orderBy: { recordedAt: "desc" },
      select: { fatMassKg: true },
    }),
    prisma.bodyMetricEntry.findFirst({
      where: { userId, deletedAt: null, muscleMassKg: { not: null } },
      orderBy: { recordedAt: "desc" },
...
async function _getMetricsAtOrBefore(userId: string, cutoff: Date) {
  const rows = await prisma.bodyMetricEntry.findMany({
    where: { userId, deletedAt: null, recordedAt: { lte: cutoff } },
    orderBy: { recordedAt: "desc" },
    take: 50,
    select: {
      weightKg: true,
  ...
  if (rows.length === 0)
    return { weightKg: null, bodyFatPct: null, fatMassKg: null, muscleMassKg: null };
...
export async function getLatestWithingsMetric(userId: string) {
  const [weight, bodyFat] = await Promise.all([
    prisma.bodyMetricEntry.findFirst({
      where: { userId, deletedAt: null, weightKg: { not: null }, source: "withings" },
      orderBy: { recordedAt: "desc" },
      select: { weightKg: true },
    }),
    prisma.bodyMetricEntry.findFirst({
      where: { userId, deletedAt: null, bodyFatPct: { not: null }, source: "withings" },
      orderBy: { recordedAt: "desc" },
...
export async function getLastNBodyMetrics(userId: string, n: number) {
  ...
  return [...byDate.entries()]
    ...
    .map(([date, entries]) => ({
      ...
      fatMassKg: avgNonNull(entries.map((e) => e.fatMassKg)),
      muscleMassKg: avgNonNull(entries.map((e) => e.muscleMassKg)),
      source: entries.some((e) => e.source === "withings")
        ? "withings"
