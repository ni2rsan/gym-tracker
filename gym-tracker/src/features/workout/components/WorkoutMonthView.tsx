...
export function WorkoutMonthView({
  anchorDate,
  data,
  selectedDate,
  onDateSelect,
  plannerBlocksByDate,
}: WorkoutMonthViewProps) {
    ...
    <div>
      ...
      <div className="grid grid-cols-7 mb-1">
        {DAY_HEADERS.map((h) => (
          <div
            key={h}
            className="text-center text-[10px] font-medium text-zinc-400 dark:text-zinc-500 py-1"
          >
            {h}
          </div>
        ))}
                ...
                {plannerBlocks.length > 0 ? (
                  plannerBlocks.every(
                    ...
                    plannerBlocks.slice(0, 3).map((b) => {
                      ...
                      return (
                        <BlockDot key={b.id} blockType={b.blockType} size="sm" status={status} />
                      );
                    })
                  )
                  ...
                  workoutMuscleGroups
                    .slice(0, 3)
                    .map((mg) => <BlockDot key={mg} blockType={mg} size="sm" status="tracked" />)
                )}
              </div>
            </button>
          );
