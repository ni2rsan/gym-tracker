...
import { MUSCLE_GROUP_LABELS } from "@/core/constants/exercises";
...
import {
  trackingIconSteps,
  trackingExerciseSteps,
} from "@/features/guide/components/trackingSteps";
import { GuideButton } from "@/features/guide/components/GuideButton";

// ── Types ──────────────────────────────────────────────────────────────────

type TrackingView = { kind: "icons" } | { kind: "exercise"; exercise: ExerciseWithSettings };
...
export function TrackingMode({
  ...
  // Refs for timer-expiry effect to avoid stale closures
  ...
  useEffect(() => {
    ...
    getExercisesComparisonBatch(batchInput, date).then((result) => {
      ...
      const newOutcomes: Record<
        string,
        { allPositive: boolean; allNegative: boolean; isPR: boolean }
      > = {};
      const newDetails: Record<
        string,
        { prevSets: PrevSet[]; currentSets: SetData[]; isPR: boolean }
      > = {};
  ...
  useEffect(() => {
    ...
    const isLast = timerSetIdx === setsRef.current.length - 1;
    setTimerSetIdx(null);
  ...
  const openExercise = (ex: ExerciseWithSettings) => {
    const prefill = sessionSavedSets[ex.id] ?? workoutData?.[ex.id] ?? makeEmptySets(ex);
  ...
  const afterSaveComparison = async (ex: ExerciseWithSettings, savedSets: SetData[]) => {
    ...
    if (compResult.success && compResult.data) {
      ...
      setComparisonDetails((prev) => ({
        ...prev,
        [ex.id]: { prevSets, currentSets: savedSets, isPR },
      }));
  ...
  const handleSavePartialSets = (ex: ExerciseWithSettings) => {
    ...
    startTransition(async () => {
      const result = await saveWorkout({
        date,
        exercises: [{ exerciseId: ex.id, sets: completedSets }],
      });
      if (result.success) {
        setCompletedIds((prev) => new Set(prev).add(ex.id));
  ...
  if (view.kind === "icons") {
    const allDone =
      nonSkippedExercises.length > 0 && nonSkippedExercises.every((ex) => completedIds.has(ex.id));
    ...
    return (
      <div className="fixed inset-0 z-[60] bg-white dark:bg-zinc-950 overflow-y-auto flex flex-col">
                  ...
                  <div className="px-4 py-3 overflow-y-auto">
                          ...
                          {diffs.map((d) => {
                            ...
                            const diffLabel = d.isNewSet ? (
                              ...
                              (() => {
                                ...
                                if (!cex.isBodyweight && d.diffKg !== null && d.diffKg !== 0)
                                  parts.push(`${d.diffKg > 0 ? "▲+" : "▼"}${fmt(d.diffKg)}kg`);
                                if (parts.length === 0)
                                  return (
                                    <span className="text-zinc-300 dark:text-zinc-600 text-[9px]">
                                      =
                                    </span>
                                  );
                                ...
                                return (
                                  <span
                                    className={`text-[9px] font-medium ${isPos ? "text-emerald-600 dark:text-emerald-400" : "text-red-500 dark:text-red-400"}`}
                                  >
                                    {parts.join(" ")}
                                  </span>
                                );
                    ...
                    {!isFirstTime && allPositive && (
                      <p className="mt-3 text-xs text-emerald-600 dark:text-emerald-400 text-center font-medium">
                        💪 Great work! You keep making progress
                      </p>
                    )}
                    {!isFirstTime && allNegative && (
                      <p className="mt-3 text-xs text-red-500 dark:text-red-400 text-center font-medium">
                        🔥 Hang in there! Keep fighting. You got this.
                      </p>
                    )}
                  </div>
                  {/* Close */}
        ...
        {summaryOverlay &&
          (() => {
            const savedSets =
              sessionSavedSets[summaryOverlay.id] ?? workoutData?.[summaryOverlay.id] ?? [];
                  ...
                  <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-100 dark:border-zinc-800">
                    <div className="flex items-center gap-2">
                      <ExerciseIcon
                        ...
                        muscleGroup={summaryOverlay.muscleGroup}
                        className="h-5 w-5"
                      />
                    ...
                    <button
                      onClick={() => setSummaryOverlay(null)}
                      className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200"
                    >
                      <X className="h-4 w-4" />
                    </button>
                    ...
                    {savedSets.length === 0 ? (
                      ...
                      savedSets.map((s) => (
                        <div key={s.setNumber} className="flex items-center gap-2 text-sm">
                          <span className="w-8 text-xs text-zinc-400 dark:text-zinc-500 font-medium shrink-0">
                            S{s.setNumber}
                          </span>
    ...
    <div className="fixed inset-0 z-[60] bg-white dark:bg-zinc-950 flex flex-col">
      ...
      {showSavePartialWarning && (
        <div className="mx-4 mt-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-700 px-4 py-3 flex items-center gap-3">
          <p className="flex-1 text-xs text-emerald-700 dark:text-emerald-300 font-medium">
            Save {completedSetFlags.filter(Boolean).length} tracked set
            {completedSetFlags.filter(Boolean).length !== 1 ? "s" : ""}?
          </p>
          <button
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col items-center gap-6">
        ...
        <div className="flex flex-col items-center gap-1">
          ...
          <p className="mt-1 text-base font-bold text-zinc-900 dark:text-white text-center">
            {ex.name}
          </p>
          <span
            className={cn(
              "text-xs font-semibold uppercase tracking-wide",
              GROUP_COLORS[ex.muscleGroup],
            )}
          >
            {MUSCLE_GROUP_LABELS[ex.muscleGroup]}
          </span>
            ...
            <>
              {sets.length > 0 && !autoTrack && (
                <div className="flex items-center gap-1.5 mb-1.5 text-xs text-zinc-400 dark:text-zinc-500">
                  ...
                  <span className={cn("text-center", ex.isBodyweight ? "flex-1" : "flex-1")}>
                    Reps
                  </span>
                ...
                if (!autoTrack) {
                  if (!isManualEditMode) {
                    return (
                      <div key={set.setNumber} className="flex items-center gap-1.5 py-1.5">
                        <span className="w-10 shrink-0 text-center text-xs text-zinc-400 dark:text-zinc-500">
                          S{set.setNumber}
                        </span>
                  ...
                  return (
                    <SetRow
                      ...
                      isBodyweight={ex.isBodyweight}
                      onChange={(updated) =>
                        setSets((prev) => prev.map((s, idx) => (idx === i ? updated : s)))
                      }
                    />
                  );
                }
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isActive &&
                        "ring-2 ring-emerald-400 dark:ring-emerald-500 bg-emerald-50 dark:bg-emerald-950/20 px-2 py-1",
                  ...
                  >
                    <div className="flex items-center gap-1.5">
                      {isActive ? (
                        ...
                        <span className="w-10 shrink-0 text-center text-emerald-500 font-bold text-sm">
                          ✓
                        </span>
                      ) : (
                          ...
                          <button
                            ...
                            onPointerDown={(e) => {
                              ...
                              setSets((prev) =>
                                prev.map((s, idx) =>
                                  idx === i
                                    ? { ...s, reps: Math.max(0, (Number(s.reps) || 0) - 1) }
                                    : s,
                                ),
                              );
                            }}
                          ...
                          <button
                            ...
                            onPointerDown={(e) => {
                              ...
                              setSets((prev) =>
                                prev.map((s, idx) =>
                                  idx === i
                                    ? { ...s, reps: Math.min(9999, (Number(s.reps) || 0) + 1) }
                                    : s,
                                ),
                              );
                            }}
                              ...
                              onPointerDown={(e) => {
                                ...
                                setSets((prev) =>
                                  prev.map((s, idx) => {
                                    ...
                                    return {
                                      ...s,
                                      weightKg: parseFloat(Math.max(0, cur - 0.5).toFixed(1)),
                                    };
                                  }),
                                );
                              }}
                              ...
                              onPointerDown={(e) => {
                                ...
                                setSets((prev) =>
                                  prev.map((s, idx) => {
                                    ...
                                    return {
                                      ...s,
                                      weightKg: parseFloat(Math.min(9999, cur + 0.5).toFixed(1)),
                                    };
                                  }),
                                );
                              }}
                            ...
                            <svg className="-rotate-90 w-full h-full" viewBox="0 0 36 36">
                              ...
                              <circle
                                ...
                                strokeDashoffset={
                                  ...
                                  Math.PI *
                                  14 *
                                  (1 - (breakDuration > 0 ? timerLeft / breakDuration : 0))
                                }
              ...
              <div className="flex items-center gap-1.5 pt-1">
                ...
                <button
                  onClick={() => {
                    setSets((prev) => [
                      ...prev,
                      { setNumber: prev.length + 1, reps: 0, weightKg: "" },
                    ]);
                    setCompletedSetFlags((prev) => [...prev, false]);
                    startTransition(async () => {
                      await savePreferredSets(ex.id, sets.length + 1);
                    });
                  }}
