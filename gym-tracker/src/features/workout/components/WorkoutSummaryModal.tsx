...
import { computeSetDiffs, computeOutcome } from "@/core/domain/workoutDiff";
import type { PrevSet, CurrSet } from "@/core/domain/workoutDiff";
...
function DiffBadge({ value, unit }: { value: number; unit: string }) {
  ...
  return (
    ...
    >
      {positive ? `▲${fmt(value)}` : `▼${fmt(Math.abs(value))}`}
      {unit}
    </span>
  );
}
...
export function WorkoutSummaryModal({
  ...
  const exerciseData = exercises.map((ex) => {
    ...
    const outcome: "pr" | "positive" | "negative" | null = ex.isPR
      ? "pr"
      : allPositive
        ? "positive"
        : allNegative
        ...
        <div className="overflow-y-auto flex-1 p-4 space-y-3">
          {exerciseData.map((ex) => {
            ...
            return (
              <div
                key={ex.exerciseId}
                className="rounded-xl bg-zinc-50 dark:bg-zinc-800/60 overflow-hidden"
              >
                {/* Exercise header */}
                      ...
                      {ex.diffs.map((d) => {
                        ...
                        return (
                          <Fragment key={d.setNumber}>
                            ...
                            <span
                              className={cn(
                                "self-center text-[10px] font-semibold text-zinc-400 dark:text-zinc-500",
                                faded,
                              )}
                            >
                              S{d.setNumber}
                            ...
                            {!ex.isBodyweight && (
                              <span
                                className={cn(
                                  ...
                                  faded,
                                  isPRSet &&
                                    "ring-1 ring-amber-400 dark:ring-amber-500 rounded-md px-1.5 py-0.5 bg-amber-50/50 dark:bg-amber-900/20",
                                )}
                              >
                                {hasCurr ? (
                                  <>
