...
import { computeSetDiffs, computeOutcome, isAssistedExercise } from "@/core/domain/workoutDiff";
...
import { MUSCLE_GROUP_ORDER, MUSCLE_GROUP_LABELS } from "@/core/constants/exercises";
import { todayISODate } from "@/core/utils/date";
import type { ExerciseWithSettings, SetData } from "@/core/types/workout";
import type { MuscleGroup } from "@/core/constants/exercises";
import type { WorkoutDayData } from "@/server/services/workoutService";
...
export function WorkoutForm({ initialExercises, initialDate }: WorkoutFormProps) {
  ...
  const [plannerData, setPlannerData] = useState<{
    blocksByDate: Record<string, PlannerBlockInfo[]>;
    sorryRemaining: number;
  } | null>(null);
  const [plannedExercisesByDate, setPlannedExercisesByDate] = useState<
    Record<string, { exerciseId: string }[]>
  >({});
  ...
  const [hiddenByGroup, setHiddenByGroup] = useState<
    Partial<Record<MuscleGroup, ExerciseWithSettings[]>>
  >({});
  ...
  const fromPlannerRef = useRef(
    !!searchParams.get("section") || searchParams.get("from") === "planner",
  );
  ...
  const applyComparisonResults = async (
    ...
    for (const ex of exList) {
      ...
      const outcome: ExOutcomeEntry["outcome"] = isPR
        ? "pr"
        : allPositive
          ? "positive"
          : allNegative
            ? "negative"
            : null;
      const diffDataMap: Record<
        number,
        { diffReps: number | null; diffKg: number | null; isPRSet: boolean }
      > = {};
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (plannedExResult.success && plannedExResult.data)
        setPlannedExercisesByDate(plannedExResult.data);
      setIsRangeLoading(false);
    }).catch(() => {
      if (!stale) setIsRangeLoading(false);
    });
    return () => { stale = true; };
  }, [rangeStart, rangeEnd]);
  ...
  const handleExercisesChanged = () => {
    Promise.all([
      ...
      getWorkoutForDate(selectedDate),
      getLastKnownSets(),
    ]).then(([exResult, hiddenResult, dateResult, lastResult]) => {
      ...
      if (hiddenResult.success && hiddenResult.data) {
        ...
        for (const h of hiddenResult.data as {
          id: string;
          name: string;
  ...
  const handleSaveFullBody = () => {
    ...
    startTransition(async () => {
      const fullBodyExercises = sortedExercises.filter(
        (ex) =>
          (ex.muscleGroup === "UPPER_BODY" || ex.muscleGroup === "LOWER_BODY") &&
  ...
  const handleSaveGroup = (mg: MuscleGroup) => {
    ...
    startTransition(async () => {
      const groupExercises = exercises.filter(
        (ex) => ex.muscleGroup === mg && !skippedIds.has(ex.id),
      );
  ...
  const handleDeleteExerciseTracking = (exerciseId: string) => {
    ...
    startTransition(async () => {
      ...
      const lastKnown = lastResult.success && lastResult.data ? lastResult.data : {};
      const prefill = initializeSets(
        [ex],
        lastKnown[exerciseId] ? { [exerciseId]: lastKnown[exerciseId]! } : {},
      );
      setSavedTodayIds((prev) => {
        const next = new Set(prev);
      ...
      setWorkoutData((prev) => ({
        ...
        [exerciseId]:
          prefill[exerciseId] ??
          makeEmptySets(ex.preferredSets ?? (ex.isBodyweight ? BODYWEIGHT_SETS : DEFAULT_SETS)),
      }));
      refreshRangeData();
    });
  };
  ...
  // Tick a timestamp every 30s so formatLastSaved stays reactive without impure Date.now() in render
  ...
  const isFutureExcusable =
    !hasWorkoutForDate && !isSelectedPast && blocksForDate.length > 0 && !allBlocksExcused;
  const sorryRemaining = plannerData?.sorryRemaining ?? 0;
  const showSorrySection =
    (isMissedDate || isFutureExcusable || allBlocksExcused) && blocksForDate.length > 0;

  // Determine which block types are due (non-excused, non-auto-promoted planned blocks)
  const dueBlockTypes = new Set(
    blocksForDate.filter((b) => !b.sorryExcused && !b.isAutoPromoted).map((b) => b.blockType),
  );
  ...
  const groupsWithPlannedExercises = new Set(
    exercises.filter((ex) => plannedExerciseIdSet.has(ex.id)).map((ex) => ex.muscleGroup),
  );
  ...
  const plannedExerciseIdsForGroup = (mg: MuscleGroup): Set<string> =>
    new Set(
      exercises
        .filter((ex) => ex.muscleGroup === mg && plannedExerciseIdSet.has(ex.id))
        .map((ex) => ex.id),
    );
    ...
    <div className="space-y-4">
      ...
      {showSorrySection && (
        <div className="flex items-center gap-2 text-xs py-0.5">
          <span className="inline-flex items-center justify-center w-4 h-4 rounded-full border-2 border-amber-400 bg-amber-100 dark:border-amber-500 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400 font-bold text-[8px] leading-none shrink-0">
            S
          </span>
          {/* Past missed */}
      ...
      {isLoading || (!!searchParams.get("section") && trackingScope === null) ? (
        ...
        <div className="space-y-4">
              ...
              <div className="flex items-center justify-between px-5 py-3.5 bg-orange-50 dark:bg-orange-950/20">
                <span className="flex items-center gap-2 font-semibold text-sm text-orange-600 dark:text-orange-400">
                  ...
                  <span className="text-xs font-normal opacity-60">
                    {(exercisesByGroup["UPPER_BODY"]?.length ?? 0) +
                      (exercisesByGroup["LOWER_BODY"]?.length ?? 0)}{" "}
                    exercises
                  </span>
                </span>
          ...
          {standaloneGroups
            .filter((mg) => dueGroupMuscles.has(mg))
            .map((mg) => (
              <div key={`${selectedDate}-${mg}`} id={`section-${mg}`}>
                <ExerciseGroup
                  ...
                  onSave={
                    isGroupEditable(mg) && exercisesByGroup[mg].length > 0
                      ? () => handleSaveGroup(mg)
                      : undefined
                  }
          ...
          {MUSCLE_GROUP_ORDER.filter((mg) => promotedGroups.has(mg)).map((mg) => (
            <div key={`${selectedDate}-promoted-${mg}`} id={`section-${mg}`}>
              <ExerciseGroup
                ...
                lastSaved={lastSavedByGroup[mg] ?? null}
                onTrack={
                  exercisesByGroup[mg].length > 0 && selectedDate <= todayISO
                    ? () => setTrackingScope(mg)
                    : undefined
                }
          ...
          {addedExercises.length > 0 && (
            <div key={`${selectedDate}-added`} id="section-added">
              <ExerciseGroup
                ...
                onSave={
                  addedExercises.length > 0
                    ? () => handleSaveAddedExercises(addedExercises)
                    : undefined
                }
          ...
          {standaloneGroups
            .filter((mg) => !effectiveDueGroups.has(mg))
            .map((mg) => (
      ...
      {trackingScope !== null &&
        (() => {
          ...
          const isWorkoutFlow =
            searchParams.get("section") === "WORKOUT" && trackingScope === "all";
            ...
            ? (() => {
                ...
                for (const mg of MUSCLE_GROUP_ORDER) {
                  if (dueGroupMuscles.has(mg) && exercisesByGroup[mg]?.length) {
                    ...
                    const addedForGroup = exercises.filter(
                      (ex) => ex.muscleGroup === mg && plannedExerciseIdSet.has(ex.id),
                    );
                    if (addedForGroup.length)
                      secs.push({ label: SECTION_LABELS[mg], exercises: addedForGroup });
                  }
                }
                if (addedExercises.length)
                  secs.push({ label: "Added Exercises", exercises: addedExercises });
            ...
            : trackingScope === "all"
              ...
              : trackingScope === "FULL_BODY"
                ? sortedExercises.filter(
                    (e) => e.muscleGroup === "UPPER_BODY" || e.muscleGroup === "LOWER_BODY",
                  )
          ...
          return (
            <TrackingMode
              ...
              onExerciseOutcome={(
                exerciseId,
                outcome,
                prevSetsFromTracking,
                currentSetsFromTracking,
              ) => {
                const outcomeStr: "positive" | "negative" | "pr" | null = outcome.isPR
                  ? "pr"
                  : outcome.allPositive
                    ? "positive"
                    : outcome.allNegative
                      ...
                      : null;
                const diffs =
                  prevSetsFromTracking && currentSetsFromTracking
