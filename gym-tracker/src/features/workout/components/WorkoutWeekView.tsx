...
import { BlockDot } from "@/features/planner/components/BlockDot";
...
export function WorkoutWeekView({
  anchorDate,
  data,
  selectedDate,
  onDateSelect,
  plannerBlocksByDate,
}: WorkoutWeekViewProps) {
          ...
          >
            ...
            <div className="relative flex flex-wrap gap-0.5 justify-center min-h-[16px]">
              {plannerBlocks.length > 0 ? (
                (() => {
                  const isAllTracked = plannerBlocks.every(
                    (b) => isBlockTracked(trackedGroups, b.blockType) || b.sorryExcused,
                  );
                  const isAnySorryExcused = plannerBlocks.some((b) => b.sorryExcused);
                  const isAnyTracked = plannerBlocks.some((b) =>
                    isBlockTracked(trackedGroups, b.blockType),
                  );
                  ...
                  return (
                    <>
                      {showSorryBadge && (
                        <span
                          className="absolute -top-1 -right-1 w-3 h-3 rounded-full border border-amber-400 bg-amber-100 dark:border-amber-500 dark:bg-amber-900/40 flex items-center justify-center pointer-events-none font-bold text-amber-600 dark:text-amber-400"
                          style={{ fontSize: "7px" }}
                        >
                          S
                        </span>
                      ...
                      {isAllTracked ? (
                        <div className="relative">
                          <BlockDot
                            blockType={plannerBlocks[0].blockType}
                            size="sm"
                            status="tracked"
                          />
                        ...
                        plannerBlocks.map((b) => {
                          ...
                          const status =
                            tracked || b.sorryExcused ? "tracked" : missed ? "missed" : undefined;
                          return (
                            <BlockDot
                              key={b.id}
