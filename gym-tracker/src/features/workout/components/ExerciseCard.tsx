    ...
    <div className="flex items-center gap-2 mb-3">
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-0.5">
        <h3 className="font-semibold text-sm text-zinc-900 dark:text-white break-words leading-tight">
          {exercise.name}
          {exercise.isPinned && (
            <span className="ml-1.5 text-emerald-500 text-xs" aria-label="Pinned">
              ★
            </span>
          )}
        </h3>
      ...
      {outcome === "pr" && <span className="text-sm leading-none shrink-0">🏆</span>}
      {outcome === "positive" && (
          ...
          <button
            onClick={() => {
              ...
              startTransition(async () => {
                await setPreferredSets(exercise.id, next.length);
              });
            }}
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "flex flex-col gap-2",
          locked && !diffData && "opacity-50 pointer-events-none",
        )}
      >
        {sets.map((set, index) => {
                ...
                <span className="flex-1 text-center text-sm text-zinc-700 dark:text-zinc-300">
                  ...
                  {diff.diffReps !== null && diff.diffReps !== 0 && (
                    <span
                      className={cn(
                        "text-[9px] ml-0.5",
                        diff.diffReps > 0 ? "text-emerald-500" : "text-red-500",
                      )}
                    >
                      {diff.diffReps > 0
                        ? `▲+${parseFloat(diff.diffReps.toFixed(1))}`
                        : `▼${parseFloat(diff.diffReps.toFixed(1))}`}
                    </span>
                  )}
                </span>
                  ...
                  <span className="flex-1 text-center text-sm text-zinc-700 dark:text-zinc-300">
                    ...
                    {diff.diffKg !== null && diff.diffKg !== 0 && (
                      <span
                        className={cn(
                          "text-[9px] ml-0.5",
                          diff.diffKg > 0 ? "text-emerald-500" : "text-red-500",
                        )}
                      >
                        {diff.diffKg > 0
                          ? `▲+${parseFloat(diff.diffKg.toFixed(1))}`
                          : `▼${parseFloat(diff.diffKg.toFixed(1))}`}
                      </span>
                    )}
                  </span>
