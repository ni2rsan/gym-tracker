...
import type { ExerciseStatCard } from "@/server/services/reportService";
...
function SetBar({
  ...
  const display = unit === "kg" ? (value % 1 === 0 ? `${value}` : value.toFixed(1)) : `${value}`;
  ...
  return (
    <div className="flex items-center gap-1 h-3.5">
      ...
      >
        {hasDiff
          ? `${diff! > 0 ? "+" : ""}${
              unit === "kg" ? (diff! % 1 === 0 ? diff : diff!.toFixed(1)) : diff
            }`
          : "0"}
      </span>
    </div>
...
function ExerciseChart({
  ...
  const allReps = filtered.flatMap((h) => h.sets.slice(0, MAX_SETS).map((s) => s.reps));
  const allKg = isBodyweight
    ? []
    : filtered.flatMap((h) => h.sets.slice(0, MAX_SETS).map((s) => s.weightKg ?? 0));
        ...
        >
          {filtered.map((workout, wIdx) => {
            ...
            const sets = [...workout.sets]
              ...
              .slice(0, MAX_SETS);
            const prevSets = prev
              ? [...prev.sets].sort((a, b) => a.setNumber - b.setNumber).slice(0, MAX_SETS)
              : null;
                        ...
                        <>
                          ...
                          {!isBodyweight && (
                            <SetBar
                              ...
                              diff={
                                prevSet != null && set.weightKg != null && prevSet.weightKg != null
            ...
            {!isCollapsed && (
              <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
                {groupCards.map((card) => {
                  ...
                  const isImprovement =
                    rawDelta !== null && (card.isAssisted ? rawDelta < 0 : rawDelta > 0);
                      ...
                      >
                        ...
                        <div className="shrink-0 text-right">
                          ...
                          <div className="flex items-center justify-end gap-1 mt-0.5 min-h-[16px]">
                            ...
                            {card.isPR && <Trophy className="h-3 w-3 text-amber-500 shrink-0" />}
                          </div>
                        </div>
