...
import type { MetricPoint } from "@/core/types/metrics";
...
export function WeightTrendChart({ data, isWithingsConnected }: WeightTrendChartProps) {
  ...
  const hasBF =
    secondaryMode === "bodyFatPct" && showSecondary && data.some((d) => d.bodyFatPct != null);
  const hasFatMass =
    secondaryMode === "composition" && showSecondary && data.some((d) => d.fatMassKg != null);
  const hasMuscleMass =
    secondaryMode === "composition" && showSecondary && data.some((d) => d.muscleMassKg != null);
  ...
  if (secondaryMode === "bodyFatPct") {
    data.forEach((d) => {
      if (d.bodyFatPct != null) rightValues.push(d.bodyFatPct);
    });
  } else {
    data.forEach((d) => {
