...
import type { TreeState, TreeStage } from "@/core/domain/gardenUtils";
import { STAGE_THRESHOLDS, TREE_CAPACITY } from "@/core/domain/gardenUtils";
        ...
        {isUnlocked && imgSrc ? (
          <>
            <img
              ...
              style={isMaxStage ? { animation: "blazeGlow 2.8s ease-in-out infinite" } : undefined}
            />
...
export function ExerciseGarden({
  stardustTotal,
  trees,
}: {
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 overflow-hidden">
        ...
        <div className="px-4 pb-3 -mt-1">
          <p className="text-[10px] text-zinc-400 dark:text-zinc-600 text-center">
            {tree1.isComplete ? (
              trees[1]?.isUnlocked ? (
                "Tending the next tree"
              ) : (
                "All trees growing — more coming soon"
              )
            ) : (
              <>
