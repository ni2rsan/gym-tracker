...
export function WorkoutCalendar({
  ...
  const [plannedExercisesByDate, setPlannedExercisesByDate] =
    useState<Record<string, PlannedExerciseInfo[]>>(initialPlannedExercises);
  ...
  const handleBlockUpdated = (blockId: string, newBlockType: string) => {
    setWorkouts((prev) =>
      prev.map((b) => (b.id === blockId ? { ...b, blockType: newBlockType } : b)),
    );
    refreshStreak();
  };
  ...
  const handleAutoPromotedBlockDeleted = (date: string, muscleGroup: string) => {
    setWorkouts((prev) =>
      prev.filter((b) => !(b.date === date && b.blockType === muscleGroup && b.isAutoPromoted)),
    );
    setContextMenu((prev) =>
      prev
        ? {
            ...prev,
            blocks: prev.blocks.filter((b) => !(b.blockType === muscleGroup && b.isAutoPromoted)),
          }
        : prev,
    );
  };
  ...
  const handleSetView = (v: View) => {
    if (v === "week") {
      ...
      const weekOff = Math.round(
        (todayMon.getTime() - week0.getTime()) / (7 * 24 * 60 * 60 * 1000),
      );
    ...
    <div className="space-y-4">
        ...
        {showNav && (
          <div className="flex items-center gap-2">
            ...
            <span
              ...
              className="text-sm font-medium text-zinc-900 dark:text-white min-w-[160px] text-center"
            >
              {headerLabel}
            </span>
            <button
      ...
      {contextMenu && (
        <DayContextMenu
          ...
          onAddBlock={() => {
            setContextMenu(null);
            setAddModalDate(contextMenu.date);
          }}
