...
import { BLOCK_LABELS, BLOCK_COLORS } from "@/core/constants/exercises";
import type { BlockType } from "@/core/constants/exercises";
import {
  deleteBlock,
  deleteSeries,
...
import {
  deleteTrackedBlockByDate,
  getWorkoutSummaryForDate,
  getFullWorkoutSummaryForDate,
} from "@/actions/workout";
import type { WorkoutExerciseSummary } from "@/server/services/workoutService";
import { WorkoutSummaryModal } from "@/features/workout/components/WorkoutSummaryModal";
import type { SummaryExerciseData } from "@/features/workout/components/WorkoutSummaryModal";
...
function isBlockTracked(groups: Set<string> | undefined, blockType: string): boolean {
  ...
  if (blockType === "FULL_BODY")
    return groups.has("UPPER_BODY") || groups.has("LOWER_BODY") || groups.has("BODYWEIGHT");
...
export function DayContextMenu({
  ...
  const [activeBlock, setActiveBlock] = useState<PlannedBlock>(
    blocks.find((b) => !b.seriesId) ?? blocks[0],
  );
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-100 dark:border-zinc-800">
        <div className="min-w-0 flex-1">
          <p suppressHydrationWarning className="text-xs text-zinc-500 dark:text-zinc-400">
            {dateLabel}
          </p>
      ...
      <div className="px-4 py-2 border-b border-zinc-100 dark:border-zinc-800">
        ...
        {activeBlock.seriesId && <span className="ml-2 text-xs text-zinc-400">Recurring</span>}
      </div>
          ...
          isMissedDay && hasStreak && sorryRemaining > 0 ? (
            ...
            <div className="px-4 py-3 space-y-2 border-t border-zinc-100 dark:border-zinc-800">
              <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-snug">
                You missed this day. Use a SORRY token to keep your{" "}
                <span className="font-bold text-amber-500">{seriesStreak}-day streak</span>.
              </p>
              <div className="flex flex-col gap-1.5">
                ...
                >
                  <span className="inline-flex items-center justify-center w-4 h-4 rounded-full border-2 border-amber-400 bg-amber-100 dark:border-amber-500 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 font-bold text-[8px] leading-none shrink-0 mr-1.5">
                    S
                  </span>
            ...
            <div className="px-4 py-2 flex items-center gap-2">
              ...
              <button onClick={() => setConfirmDelete(null)} className="text-xs text-zinc-400">
                No
              </button>
            </div>
          )
          ...
          <div className="border-t border-zinc-100 dark:border-zinc-800">
            ...
            {isMissedDay && !confirmSorryToken && sorryRemaining === 0 && (
              <div className="px-4 py-2.5 text-sm text-zinc-400 dark:text-zinc-500 flex items-center gap-2">
                <span className="inline-flex items-center justify-center w-4 h-4 rounded-full border-2 border-zinc-400 bg-zinc-200 dark:border-zinc-600 dark:bg-zinc-700 text-zinc-400 dark:text-zinc-500 font-bold text-[8px] leading-none shrink-0">
                  S
                </span>
        ...
        {workedOut &&
          (confirmDelete === "workout" ? (
            ...
            >
              <Trash2 className="h-4 w-4" />
              Delete tracked{" "}
              {BLOCK_LABELS[activeBlock.blockType as BlockType] ?? activeBlock.blockType}
            </button>
          ))}
      </div>
