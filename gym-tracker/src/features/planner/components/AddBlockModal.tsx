...
import { BLOCK_LABELS, BLOCK_COLORS, BlockType } from "@/core/constants/exercises";
import {
  createBlock,
  createSeries,
  updateBlock,
          ...
          <>
            <div className="grid grid-cols-2 gap-2">
              {(Object.entries(BLOCK_LABELS) as [string, string][]).map(([type, label]) => (
                ...
                >
                  <span
                    className={cn("w-3 h-3 rounded-full shrink-0", BLOCK_COLORS[type as BlockType])}
                  />
                  {label}
                </button>
              ))}
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/40 p-4 text-center">
              ...
              <p className="text-sm font-semibold text-amber-800 dark:text-amber-200">
                You have a{" "}
                <span className="text-amber-500 font-black">{seriesStreak}-day streak</span>!
              </p>
            ...
            {sorryRemaining > 0 ? (
              ...
              >
                <span className="inline-flex items-center justify-center w-4 h-4 rounded-full border-2 border-white/60 bg-white/20 font-bold text-[8px] leading-none shrink-0 mr-1.5">
                  S
                </span>
