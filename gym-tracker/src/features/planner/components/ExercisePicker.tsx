...
import {
  ...
  createAutoPromotedBlock,
  deleteAutoPromotedBlock,
...
export function ExercisePicker({
  ...
  const [exercises, setExercises] = useState<
    { id: string; name: string; muscleGroup: string; isBodyweight: boolean }[]
  >([]);
  const [loadingExercises, setLoadingExercises] = useState(true);
  const [planned, setPlanned] = useState<PlannedExerciseInfo[]>(plannedExercises);
  const [localAutoPromoted, setLocalAutoPromoted] = useState<Set<string>>(
    new Set(autoPromotedGroups),
  );
  ...
  const filtered = exercises.filter(
    (e) =>
      e.name.toLowerCase().includes(query.toLowerCase()) && !blockedMuscleGroups.has(e.muscleGroup),
  );

  const grouped = GROUP_ORDER.reduce<Record<string, typeof filtered>>(
    (acc, g) => {
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-zinc-900 rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[85vh]">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-100 dark:border-zinc-800 shrink-0">
          <div>
            ...
            <p suppressHydrationWarning className="text-xs text-zinc-400 dark:text-zinc-500">
              {dateLabel}
            </p>
          </div>
          <button
