  ...
  function confirmNoteAction() {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        setRequests((prev) =>
          prev.map((r) =>
            r.id === id ? { ...r, status: action, adminNote: noteText.trim() || null } : r,
          ),
        );
        setPendingAction(null);
        setNoteText("");
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  ...
                  <span
                    className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[11px] font-semibold shrink-0 ${statusCfg.className}`}
                  >
                  ...
                  <div className="mt-3 flex flex-col gap-2 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/50 p-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        {isPending
                          ? "Saving…"
                          : `Confirm ${pendingAction.action === "DECLINED" ? "Decline" : "Deploy"}`}
                      </button>
                      <button
