...
import { AddFriendForm } from "@/features/social/components/AddFriendForm";
import { FriendRequestCard } from "@/features/social/components/FriendRequestCard";
import { GlobalPrivacySettings } from "@/features/social/components/GlobalPrivacySettings";
import {
  removeFriend,
  upsertFriendPrivacyOverride,
  toggleFistBump,
  markSocialSeen,
} from "@/actions/social";
import dynamic from "next/dynamic";
const FriendProfileView = dynamic(
  () => import("@/features/social/components/FriendProfileView").then((m) => m.FriendProfileView),
  {
    ssr: false,
    loading: () => (
...
import type {
  FriendProfileData,
  WorkoutFeedEntry,
  NewFistBumpNotification,
  SocialStats,
} from "@/core/types/social";
...
function FeedCard({
  ...
  const displayName = entry.isOwnWorkout ? "You" : (entry.username ?? entry.name ?? "Someone");
  ...
  return (
    <div
      className={`rounded-2xl border bg-white dark:bg-zinc-900 transition-colors ${highlightClass}`}
    >
          ...
          <div className="flex items-center justify-between mt-1.5">
            ...
            {entry.isOwnWorkout && bumpCount > 0 && (
              <span className="flex items-center gap-1 text-xs text-zinc-500 dark:text-zinc-400 max-w-[160px]">
                <img
                  src="/fistbumpicon.png"
                  alt=""
                  className="h-3.5 w-3.5 object-contain flex-shrink-0 fb-icon"
                />
          ...
          {entry.exercises.map((ex) => (
            <div key={ex.exerciseName}>
              <div className="flex items-center gap-1.5 mb-1">
                <span
                  className={`w-1.5 h-1.5 rounded-full shrink-0 ${MG_DOT[ex.muscleGroup] ?? "bg-zinc-400"}`}
                />
              ...
              <div className="flex flex-wrap gap-1 pl-3">
                {ex.sets.map((s) => (
                  ...
                  >
                    <span className="text-zinc-400 dark:text-zinc-500 text-[9px] font-bold mr-0.5">
                      {s.setNumber}
                    </span>
                    {s.weightKg != null
    ...
    <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-hidden">
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          ...
          <div>
            <p className="text-base font-bold text-zinc-900 dark:text-white">
              {data.totalWorkoutsThisMonth}
            </p>
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5">This month</p>
          </div>
          <div>
            <p className="text-base font-bold text-zinc-900 dark:text-white">
              {new Date(data.joinedAt + "T00:00:00").toLocaleDateString("en-US", {
                month: "short",
                year: "numeric",
              })}
            </p>
...
export function SocialPageClient({
  ...
  const [newBumpSessionIds, setNewBumpSessionIds] = useState(
    () => new Set(newFistBumps.map((b) => b.sessionId)),
  );
  ...
  useEffect(() => {
    if (view === "main" && tab === "feed") {
      ...
      if (fistBumpBadge > 0) {
        const names = [
          ...new Set(newFistBumps.map((b) => b.bumperUsername ?? b.bumperName ?? "Someone")),
        ];
  ...
  const handleFistBump = useCallback(
    (sessionId: string) => {
      // Capture pre-toggle state for surgical revert on error
      let snapshot: typeof feedState | null = null;
      setFeedState((prev) => {
        snapshot = prev;
      ...
      toggleFistBump(sessionId).then((result) => {
        if (!result.success && snapshot) {
          setFeedState(snapshot); // revert to pre-toggle state only
        }
      });
    },
    [],
  ...
  const overlayText =
    overlayBumpers.length === 0
      ? ""
      : overlayBumpers.length === 1
        ? overlayBumpers[0]
        : overlayBumpers.length === 2
          ? `${overlayBumpers[0]} & ${overlayBumpers[1]}`
    ...
    return (
      <div className="space-y-6">
            ...
            <div className="space-y-2">
              {pendingState.map((req) => (
                <FriendRequestCard
                  ...
                  friendshipId={req.id}
                  sender={req.sender}
                  onAccepted={() => handleRequestActioned(req.id)}
                  onDeclined={() => handleRequestActioned(req.id)}
                />
              ))}
            </div>
          </div>
        ...
        {pendingSent.length > 0 && (
          <div className="space-y-2">
            <h2 className="text-sm font-semibold text-zinc-500 dark:text-zinc-400 px-0.5">
              Sent Requests
            </h2>
    ...
    <div className="space-y-5">
            ...
            >
              ...
              {fistBumpBadge > 0 && (
                <span className="absolute -top-2 -right-1 flex items-center gap-px text-[8px] font-bold leading-none">
                  <img src="/fistbumpicon.png" alt="" className="h-5 w-5 object-contain fb-icon" />
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 p-3 text-center">
          <Users
            className="h-4 w-4 text-zinc-400 dark:text-zinc-500 mx-auto mb-1"
            strokeWidth={1.5}
          />
          <div className="text-xl font-bold text-zinc-900 dark:text-white tabular-nums">
            {friends.length}
          </div>
        ...
        <div className="rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 p-3 text-center">
          <img
            src="/fistbumpicon.png"
            alt=""
            className="h-4 w-4 object-contain mx-auto mb-1 fb-icon"
          />
          <div className="text-xl font-bold text-zinc-900 dark:text-white tabular-nums">
            {socialStats.totalFistBumpsReceived}
          </div>
        ...
        <div className="rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 p-3 text-center">
          ...
          />
          <div className="text-xl font-bold text-zinc-900 dark:text-white tabular-nums">
            {socialStats.totalWorkoutsTracked}
          </div>
