...
function CompactBadge3D({ path, title, tag }: { path: string; title: string; tag: string }) {
  return (
    <div className="flex flex-col items-center gap-0.5">
      ...
      <span className="text-[10px] font-semibold text-zinc-700 dark:text-zinc-300 leading-none">
        {title}
      </span>
    </div>
  );
            ...
            <div key={group}>
              ...
              <div className="grid grid-cols-3 gap-1.5">
                {grouped[group].map((pr) => (
                  <div
                    key={pr.exerciseId}
                    className="bg-zinc-50 dark:bg-zinc-800 rounded-lg p-1.5 flex flex-col items-center gap-0.5 min-w-0"
                  >
                    <ExerciseIcon
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className={`flex h-6 w-11 items-center rounded-full px-0.5 transition-colors disabled:opacity-50 ${
            effective ? "bg-emerald-500 justify-end" : "bg-zinc-200 dark:bg-zinc-700 justify-start"
          }`}
        >
      ...
      <div className="space-y-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              {data.name && data.username && (
                <p className="text-sm text-zinc-500 dark:text-zinc-400">{data.name}</p>
              )}
            </div>
          </div>
        )}
                  ...
                  {data.heightCm != null && (
                    <div className="px-4 py-3">
                      ...
                      <p className="text-xl font-bold text-zinc-900 dark:text-white">
                        {data.heightCm}
                        <span className="text-xs font-normal text-zinc-400 ml-1">cm</span>
                      </p>
                    </div>
                  )}
                  {data.visibility.canSeeWeight && (
        ...
        <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-hidden">
          ...
          <div className="px-3 py-3 space-y-4">
            ...
            {(() => {
              const unlocked = WORKOUT_MILESTONES.filter((d) =>
                data.milestonesUnlocked.includes(d),
              );
              ...
              return (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {unlocked.map((days) => (
                      <div key={days} className="flex flex-col items-center w-14">
                        ...
                        />
                        <span className="-mt-1 text-[8px] font-semibold text-zinc-500 dark:text-zinc-400 leading-tight">
                          {days}d
                        </span>
                      </div>
                    ))}
            ...
            {(() => {
              const unlocked = VOLUME_BADGES.filter(
                (b) => (data.cumulativeVolume ?? 0) >= b.thresholdKg,
              );
              if (unlocked.length === 0) return null;
              return (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {unlocked.map((b) => (
                      <div key={b.key} className="flex flex-col items-center w-10">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img
            ...
            {(() => {
              const achievedFriends = FRIEND_BADGES_DEF.filter(
                (b) => (data.friendCount ?? 0) >= b.threshold,
              );
              const achievedFistbumps = FISTBUMP_BADGES_DEF.filter(
                (b) => (data.fistbumpCount ?? 0) >= b.threshold,
              );
        ...
        {/* PRs */}
        {data.visibility.canSeePRs && data.prs.length > 0 && <PRSection prs={data.prs} />}
