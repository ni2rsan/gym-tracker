  ...
  return (
    <div className="w-full max-w-[260px] rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-hidden shadow-sm">
      ...
      <div className="flex items-start justify-between gap-2 px-3 py-2 border-b border-zinc-100 dark:border-zinc-800 flex-wrap">
        ...
        <div className="flex rounded-lg border border-zinc-200 dark:border-zinc-700 overflow-hidden text-[9px] font-medium w-fit">
          <div className="px-2 py-1 bg-zinc-900 text-white dark:bg-white dark:text-zinc-900">
            Body Fat %
          </div>
    ...
    <div className="w-full max-w-[260px] rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-hidden shadow-sm">
        ...
        <div className="flex flex-wrap gap-1.5">
          {[
            { label: "Upper Body", active: true, cls: "bg-blue-600 text-white border-blue-600" },
            {
              label: "Lower Body",
              active: false,
              cls: "border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-400",
            },
            {
              label: "Bodyweight",
              active: false,
            ...
            <span
              key={label}
              className={`rounded-lg px-2 py-1 text-[9px] font-semibold border ${cls}`}
            >
              {label}
            </span>
          ))}
      ...
      <div className="px-3 py-2">
        ...
        <div className="flex justify-between mt-1">
          {["Feb", "", "", "", "", "", "Mar"].map((l, i) => (
            <span key={i} className="text-[7px] text-zinc-400 flex-1 text-center">
              {l}
            </span>
          ))}
        </div>
        <div className="mt-1 flex items-center gap-1.5">
          ...
          <span className="text-[9px] text-zinc-500">
            Max: <span className="font-semibold text-zinc-900 dark:text-white">80 kg</span>
          </span>
        </div>
      </div>
    </div>
...
export const reportsSteps: GuideStep[] = [
  {
    title: "Weight Trend",
    description:
      "Log your body weight from the Tracker page. The chart here shows your trend over 30, 60, or 90 days. Toggle Body Fat and Muscle Mass if you have a Withings scale connected.",
    illustration: <WeightTrendIllustration />,
  },
  {
    title: "Exercise Progress",
    description:
      "Pick any exercise from the dropdown to see your max weight lifted over time. Great for spotting plateaus and celebrating new bests.",
    illustration: <ExerciseProgressIllustration />,
  },
  {
    title: "Connect Withings",
    description:
      "Link your Withings smart scale to auto-import body weight, body fat %, and muscle mass. Readings sync automatically every time you open Reports.",
