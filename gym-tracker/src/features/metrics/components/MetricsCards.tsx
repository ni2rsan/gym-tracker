...
function Trend({
  ...
  return (
    <span
      className={`flex items-center gap-0.5 text-xs font-medium ${isUp ? "text-blue-500" : "text-orange-500"}`}
    >
...
export function MetricsCards({
  ...
  const handleSave = (field: EditField) => {
    const data = field === "weight" ? { weightKg: weightValue } : { bodyFatPct: bodyFatValue };

    startTransition(async () => {
      const result = await addBodyMetric(data);
      if (result.success) {
        setToast({
          message: `${field === "weight" ? "Weight" : "Body fat"} updated ✓`,
          type: "success",
        });
        setEditing(null);
        setWeightValue("");
              ...
              <div className="flex flex-col gap-1">
                ...
                {weightSource !== "withings" && withingsWeight != null && (
                  <button
                    ...
                    disabled={isPending}
                    className="flex items-center gap-1 text-xs text-zinc-400 hover:text-emerald-500 transition-colors disabled:opacity-50 w-fit mt-1"
                  >
              ...
              <div className="px-4 py-3 flex flex-col gap-1">
                ...
                <p className="text-lg font-bold text-zinc-900 dark:text-white">
                  {currentFatMassKg != null ? (
                    <>
                      {Number(currentFatMassKg).toFixed(1)}
                ...
                <Trend
                  current={currentFatMassKg}
                  previous={rangeAgoFatMassKg}
                  unit="kg"
                  rangeLabel={rangeLabel}
                />
              </div>
              <div className="px-4 py-3 flex flex-col gap-1">
                ...
                <p className="text-lg font-bold text-zinc-900 dark:text-white">
                  {currentMuscleMassKg != null ? (
                    <>
                      {Number(currentMuscleMassKg).toFixed(1)}
                      <span className="text-xs font-normal text-zinc-400 ml-0.5">kg</span>
                    </>
                  ) : (
                    <span className="text-zinc-400 text-base">—</span>
                ...
                <Trend
                  current={currentMuscleMassKg}
                  previous={rangeAgoMuscleMassKg}
                  unit="kg"
                  rangeLabel={rangeLabel}
                />
              </div>
            ...
            <div className="rounded-xl border border-dashed border-zinc-200 dark:border-zinc-700 px-4 py-3">
              <label className="flex items-center gap-2.5 cursor-pointer select-none">
                <input
                  ...
                  checked={showBodyFat}
                  onChange={(e) => {
                    setShowBodyFat(e.target.checked);
                    setEditing(null);
                  }}
